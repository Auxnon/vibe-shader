...
vec3 renderScene(vec2 uv) {
    ...
    if(d < MAX_DIST) {
        ...
        if(tetDist < SURF_DIST * 2.0) {
            ...
            float ior = 1.5;
            vec3 refractRayR = refract(rd, n, 1.0 / (ior + 0.03));
            vec3 refractRayG = refract(rd, n, 1.0 / ior);
            vec3 refractRayB = refract(rd, n, 1.0 / (ior - 0.03));
            ...
            for(int channel = 0; channel < 3; channel++) {
                ...
                float iorExit = channel == 0 ? (ior + 0.03) : (channel == 1 ? ior : (ior - 0.03));
                ...
                if(exitD < MAX_DIST) {
                    ...
                    if(exitTetDist > SURF_DIST * 2.0) {
                        // Hit the plane - smoother background pattern
                        refractedColor[channel] = 0.6;
                    } else {
                        // Hit background
                        refractedColor[channel] = 0.08;
                    }
                } else {
                    // Hit nothing - use background gradient
                    float bgGrad = mix(0.1, 0.02, uv.y * 0.5 + 0.5);
                    refractedColor[channel] = bgGrad;
                }
            }
            ...
            col = mix(refractedColor, reflectCol, fresnel * 0.35);
            ...
            col += vec3(0.3, 0.4, 0.6) * edge * 0.15;
            ...
            // Base plane color - smooth gradient instead of harsh checkerboard
            float checker = smoothstep(0.4, 0.6, sin(p.x * PI) * sin(p.z * PI) * 0.5 + 0.5);
            vec3 planeColor = mix(vec3(0.03, 0.03, 0.06), vec3(0.06, 0.06, 0.10), checker);
            
            col = planeColor * (dif * 0.6 + 0.4);
        }
    } else {
        // Background gradient
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Simple anti-aliasing with single offset sample
    vec2 pixelSize = vec2(1.0) / iResolution.xy;
    vec2 offset = vec2(0.25) * pixelSize;
    
    vec3 col = renderScene(uv + offset);
